...
void init_graphe(graphe *g, char *file_name){
    ...
    //Ouverture du fichier du graphe
    ...
    err = fscanf(fd, "%s", tmp_char); //Lecture de la châine n_sommets
    if(err == -1){
        perror("Erreur lecture châine n_sommets");
        exit(EXIT_FAILURE);
    }
    err = fscanf(fd, "%d", &val); //Lecture de la valeur n_sommets
    if(err == -1){
        perror("Erreur lecture valeur n_sommets");
        exit(EXIT_FAILURE);
    }
    ...
    //Allocation de la mémoire nécessaire pour la matrice d'adjacence
    ...
    //Initialisation des données orienté
    err = fscanf(fd, "%s", tmp_char);
    err = fscanf(fd, "%d", &val);
    g->oriente = val;

    //Initialisation des données valué
    err = fscanf(fd, "%s", tmp_char);
    err = fscanf(fd, "%d", &val);
    g->value = val;
    
    err = fscanf(fd, "%s", tmp_char);//Lecture de la châine DEBUT_DEF_ARETES
    
    //Lectures des liste d'adjacences ainsi que des matrices d'adjacences
